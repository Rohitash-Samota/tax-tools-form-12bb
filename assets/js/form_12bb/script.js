(function () {
  ...
  ready(function () {
    ...
    function showPdfUI(formId) {
      var base = "/tax-tools/form-12bb/pdf/" + formId;
      $("#pdfFrame").attr("src", base + "?disposition=inline");
      $("#btnPdfDownload").attr("href", base + "?disposition=download");

      $(".tab-container").hide();
      $("#pdfContainer").show();
    }

    $(document).on("click", "#btnPdfBack", function () {
      $("#pdfContainer").hide();
      $(".tab-container").show();
    });

    $(document).on("click", "#btnPdfPrint", function () {
      var src = $("#pdfFrame").attr("src");
